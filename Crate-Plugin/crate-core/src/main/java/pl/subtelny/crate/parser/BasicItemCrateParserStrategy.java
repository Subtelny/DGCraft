...
import pl.subtelny.crate.type.basic.BasicItemCrate;
...
public class BasicItemCrateParserStrategy extends AbstractFileParserStrategy<ItemCrate> implements ItemCrateParserStrategy {
    ...
    private List<Reward> loadRewards(String path) {
        Set<String> conditionPaths = ConfigUtil.getSectionKeys(configuration, path + ".rewards").orElseGet(HashSet::new);
    ...
    private List<CostCondition> loadCostConditions(String path) {
        Set<String> conditionPaths = ConfigUtil.getSectionKeys(configuration, path + ".cost-conditions").orElseGet(HashSet::new);
        return conditionPaths.stream()
    ...
    private List<Condition> loadConditions(String path) {
        Set<String> conditionPaths = ConfigUtil.getSectionKeys(configuration, path + ".conditions").orElseGet(HashSet::new);
        return conditionPaths.stream()
