/*
 * This file is generated by jOOQ.
 */
package pl.subtelny.islands.generated.tables.records;


import java.sql.Timestamp;

import javax.annotation.processing.Generated;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record4;
import org.jooq.Row4;
import org.jooq.impl.UpdatableRecordImpl;

import pl.subtelny.islands.generated.enums.Islandtype;
import pl.subtelny.islands.generated.tables.Islands;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.12.3"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class IslandsRecord extends UpdatableRecordImpl<IslandsRecord> implements Record4<Integer, Timestamp, String, Islandtype> {

    private static final long serialVersionUID = -1803088172;

    /**
     * Setter for <code>public.islands.id</code>.
     */
    public void setId(Integer value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.islands.id</code>.
     */
    public Integer getId() {
        return (Integer) get(0);
    }

    /**
     * Setter for <code>public.islands.created_date</code>.
     */
    public void setCreatedDate(Timestamp value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.islands.created_date</code>.
     */
    public Timestamp getCreatedDate() {
        return (Timestamp) get(1);
    }

    /**
     * Setter for <code>public.islands.spawn</code>.
     */
    public void setSpawn(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.islands.spawn</code>.
     */
    public String getSpawn() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.islands.type</code>.
     */
    public void setType(Islandtype value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.islands.type</code>.
     */
    public Islandtype getType() {
        return (Islandtype) get(3);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Integer> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record4 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row4<Integer, Timestamp, String, Islandtype> fieldsRow() {
        return (Row4) super.fieldsRow();
    }

    @Override
    public Row4<Integer, Timestamp, String, Islandtype> valuesRow() {
        return (Row4) super.valuesRow();
    }

    @Override
    public Field<Integer> field1() {
        return Islands.ISLANDS.ID;
    }

    @Override
    public Field<Timestamp> field2() {
        return Islands.ISLANDS.CREATED_DATE;
    }

    @Override
    public Field<String> field3() {
        return Islands.ISLANDS.SPAWN;
    }

    @Override
    public Field<Islandtype> field4() {
        return Islands.ISLANDS.TYPE;
    }

    @Override
    public Integer component1() {
        return getId();
    }

    @Override
    public Timestamp component2() {
        return getCreatedDate();
    }

    @Override
    public String component3() {
        return getSpawn();
    }

    @Override
    public Islandtype component4() {
        return getType();
    }

    @Override
    public Integer value1() {
        return getId();
    }

    @Override
    public Timestamp value2() {
        return getCreatedDate();
    }

    @Override
    public String value3() {
        return getSpawn();
    }

    @Override
    public Islandtype value4() {
        return getType();
    }

    @Override
    public IslandsRecord value1(Integer value) {
        setId(value);
        return this;
    }

    @Override
    public IslandsRecord value2(Timestamp value) {
        setCreatedDate(value);
        return this;
    }

    @Override
    public IslandsRecord value3(String value) {
        setSpawn(value);
        return this;
    }

    @Override
    public IslandsRecord value4(Islandtype value) {
        setType(value);
        return this;
    }

    @Override
    public IslandsRecord values(Integer value1, Timestamp value2, String value3, Islandtype value4) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached IslandsRecord
     */
    public IslandsRecord() {
        super(Islands.ISLANDS);
    }

    /**
     * Create a detached, initialised IslandsRecord
     */
    public IslandsRecord(Integer id, Timestamp createdDate, String spawn, Islandtype type) {
        super(Islands.ISLANDS);

        set(0, id);
        set(1, createdDate);
        set(2, spawn);
        set(3, type);
    }
}
